...
const ProfileView: React.FC<ProfileViewProps> = ({ user, posts, onTogglePrivacy, onFollow, onOpenSettings }) => {
  const [isFollowing, setIsFollowing] = useState(false);

  const handleFollow = () => {
    const nextState = !isFollowing;
    setIsFollowing(nextState);
    if (nextState && onFollow) {
      onFollow(user.id);
    }
        ...
        <div className="w-full space-y-3">
          <div className="flex gap-3">
             <button 
               ...
               className={`flex-1 h-14 rounded-[2rem] font-black text-xs tracking-[0.3em] uppercase transition-all shadow-2xl flex items-center justify-center gap-3 active:scale-95 ${isFollowing ? 'bg-white/5 text-white border border-white/10' : 'bg-[#00f2ff] text-black shadow-[0_0_20px_rgba(0,242,255,0.4)]'}`}
             >
               {isFollowing ? 'SYNCED' : 'ESTABLISH LINK'}
               {isFollowing && <div className="w-2 h-2 rounded-full bg-[#00f2ff] animate-pulse" />}
             </button>
            ...
            <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]">
               <div className="flex gap-4">
                  ...
                  <div className="flex flex-col items-center gap-1">
                    ...
                    <span className="text-[10px] font-black text-white">{post.resonations}</span>
                  </div>
               </div>
            </div>
          </div>
